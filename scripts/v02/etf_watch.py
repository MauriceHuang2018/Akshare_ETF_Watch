...
def _parse_fund_managers(raw: str | None) -> tuple:
    """Parse comma-separated fund manager names; default to four managers."""
    if raw is None or not str(raw).strip():
        return _DEFAULT_FUND_MANAGERS
    parts = [p.strip() for p in str(raw).split(",") if p.strip()]
    return tuple(parts) if parts else _DEFAULT_FUND_MANAGERS


def _expand_manager_match_tokens(manager: str) -> tuple:
    """Return name substrings used to match a fund manager in ETF short name."""
    return _FUND_MANAGER_ALIASES.get(manager, (manager,))


def _build_manager_matchers(managers: tuple) -> dict:
    """Build manager -> match tokens map for whitelist filtering."""
    return {m: _expand_manager_match_tokens(m) for m in managers}


def _match_fund_manager(name: str, matchers: dict) -> str | None:
    """Return canonical manager name if ETF name matches whitelist tokens."""
    text = str(name or "")
    for manager, tokens in matchers.items():
        for token in tokens:
            if token in text:
                return manager
    return None
...
def _is_allowed_fund_manager(name: str, matchers: dict) -> bool:
    """True if ETF short name matches any configured fund manager."""
    return _match_fund_manager(name, matchers) is not None


def _default_themes_path(skill_root: Path) -> Path:
    """Default path to config/themes.json under skill root."""
    return skill_root / "config" / "themes.json"


def _load_themes_config(themes_file: Path) -> dict:
    """Load themes.json; return empty dict if missing."""
    raw = _load_json(themes_file)
    return raw if isinstance(raw, dict) else {}


def _theme_key_exact(name: str, themes_cfg: dict | None = None, use_config_strip: bool = True) -> str:
    """Normalize ETF name to a theme grouping key."""
    s = str(name or "").strip()
    for token in ("ETF", "etf", "基金", "联接", "LOF", "交易型开放式指数"):
        s = s.replace(token, "")
    for token in _FUND_MANAGER_TOKENS:
        s = s.replace(token, "")
    if use_config_strip and themes_cfg:
        for suffix in themes_cfg.get("strip_suffixes", []):
            while s.endswith(suffix):
                s = s[: -len(suffix)]
        for prefix in themes_cfg.get("strip_prefixes", []):
...
def _resolve_theme_id(name: str, themes_cfg: dict) -> tuple:
    """
    Map ETF name to (theme_id, theme_label, normalized_key).
    Longest synonym match wins; fallback theme_id is normalized key.
    """
    normalized = _theme_key_exact(name, themes_cfg, use_config_strip=True)
    source = str(name or "")
    best_len = 0
    best_theme = None
    for theme in themes_cfg.get("themes", []):
        for match in theme.get("match", []):
            if match in source or match in normalized:
                if len(match) > best_len:
                    best_len = len(match)
                    best_theme = theme
    if best_theme:
        return best_theme["id"], best_theme.get("label", best_theme["id"]), normalized
    return normalized, normalized, normalized


def _dedupe_by_theme(items: list, mode: str, themes_cfg: dict) -> tuple:
    """Second-level dedupe by theme key; returns (selected, merged_count)."""
    from collections import defaultdict

    if mode == "off":
        for item in items:
            tid, label, _ = _resolve_theme_id(item.get("name", ""), themes_cfg)
            item["theme_id"] = tid
            item["theme_label"] = label
            item["theme_peer_count"] = 1
        return items, 0

    groups = defaultdict(list)
    for item in items:
        if mode == "exact":
            key = _theme_key_exact(item.get("name", ""), themes_cfg, use_config_strip=False)
            item["theme_id"] = key
            item["theme_label"] = key
        else:
            tid, label, _ = _resolve_theme_id(item.get("name", ""), themes_cfg)
            item["theme_id"] = tid
            item["theme_label"] = label
    ...
    for _key, group in groups.items():
        ...
        best["theme_peer_count"] = len(group)
        if len(group) > 1:
            merged += len(group) - 1
...
def _cache_dir(skill_root: Path, week_key: str) -> Path:
    """Path to this week's v0.2 cache directory."""
    return skill_root / "cache" / "v02" / week_key
...
def _refresh_static_data(
    ...
    fund_managers: tuple = _DEFAULT_FUND_MANAGERS,
    theme_dedupe: str = _DEFAULT_THEME_DEDUPE,
    themes_cfg: dict = None,
) -> tuple:
    """
    Fetch spot, tracking (overview per code), scale; build universe and selected; write static cache.

    Returns:
        (selected list, universe_count, excluded dict, after_index_count, after_theme_count)
    """
    if themes_cfg is None:
        themes_cfg = {}
    matchers = _build_manager_matchers(fund_managers)
    ...
    for _, row in spot.iterrows():
        ...
        if not _is_allowed_fund_manager(name, matchers):
            continue
    ...
    universe, excluded = _build_universe(spot, tracking_map, scale_map, matchers)
    by_index, all_count = _dedupe_by_index(universe)
    after_index_count = len(by_index)
    selected, theme_merged = _dedupe_by_theme(by_index, theme_dedupe, themes_cfg)
    after_theme_count = len(selected)
    ...
    _save_json(cache_universe, {
        ...
        "after_index_dedupe_count": after_index_count,
        "after_theme_dedupe_count": after_theme_count,
        "excluded": excluded,
        "items": universe,
    ...
    meta.update({
        ...
        "fund_managers": list(fund_managers),
        "theme_dedupe": theme_dedupe,
        "themes_version": themes_cfg.get("version"),
        "static_refreshed_at": datetime.now().isoformat(),
    ...
    return selected, all_count, excluded, after_index_count, after_theme_count
...
def _build_universe(spot: pd.DataFrame, tracking_map: dict, scale_map: dict, matchers: dict) -> tuple:
    """
    Build filtered ETF records with tracking index and scale.
    Drops no-tracking, value-style, non-whitelist managers; keeps cross-border.
    """
    code_col = "代码" if "代码" in spot.columns else "基金代码"
    name_col = "名称" if "名称" in spot.columns else "基金简称"

    universe = []
    excluded = {"no_tracking": 0, "value_style": 0, "fund_manager": 0}
    for _, row in spot.iterrows():
        code = _normalize_code(row[code_col])
        name = str(row.get(name_col, "") or "")
        manager = _match_fund_manager(name, matchers)
        if not manager:
            excluded["fund_manager"] += 1
            continue
...
def run_pipeline(
    ...
    fund_managers: tuple = _DEFAULT_FUND_MANAGERS,
    theme_dedupe: str = _DEFAULT_THEME_DEDUPE,
    themes_file: Path = None,
) -> dict:
    """Main pipeline: static (weekly) -> daily hist -> classify -> JSON result."""
    themes_path = themes_file or _default_themes_path(skill_root)
    themes_cfg = _load_themes_config(themes_path)
    ...
    excluded = None
    all_count = None
    after_index_count = None
    after_theme_count = None
    if refresh_static:
        static_mode = "static_full"
        selected, all_count, excluded, after_index_count, after_theme_count = _refresh_static_data(
            cache_root,
            week_key,
            force_tracking=True,
        ...
        if not selected:
            ...
            u = _load_json(cache_universe) or {}
            after_index_count = u.get("after_index_dedupe_count")
            after_theme_count = u.get("after_theme_dedupe_count")
    ...
    for item in selected:
        ...
        entry = {
            ...
            "theme_peer_count": item.get("theme_peer_count"),
    ...
    if after_index_count is not None:
        result["summary"]["after_index_dedupe_count"] = after_index_count
    if after_theme_count is not None:
        result["summary"]["after_theme_dedupe_count"] = after_theme_count
    if all_count is not None:
        ...
        if after_index_count is None:
            result["summary"]["after_index_dedupe_count"] = u.get("after_index_dedupe_count")
        if after_theme_count is None:
            result["summary"]["after_theme_dedupe_count"] = u.get("after_theme_dedupe_count")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated fund manager whitelist (default: 华夏,易方达,国泰,景顺长城)",
    )
    parser.add_argument(
        "--theme-dedupe",
        choices=_VALID_THEME_DEDUPE,
        default=_DEFAULT_THEME_DEDUPE,
        help="Second-level dedupe by theme: off|exact|enhanced (default: enhanced)",
    )
    parser.add_argument(
        "--themes-file",
        type=str,
        default=None,
        help="Override path to config/themes.json",
    )
    args = parser.parse_args()

    skill_root = _skill_root()
    fund_managers = _parse_fund_managers(args.fund_managers)
    themes_file = Path(args.themes_file) if args.themes_file else None
    try:
        result = run_pipeline(
            skill_root,
            ...
            fund_managers=fund_managers,
            theme_dedupe=args.theme_dedupe,
